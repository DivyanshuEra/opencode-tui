...
describe("FileReferenceManager", () => {
  ...
  describe("expandDirectory", () => {
    ...
    it("returns an empty array when the path does not exist", async () => {
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(path.normalize("/workspace/missing"));
    });
    ...
    it("returns an empty array when the path is not a directory", async () => {
      ...
      expect(fs.statSync).toHaveBeenCalledWith(path.normalize("/workspace/README.md"));
    });
    ...
    it("recursively expands a relative directory, ignores common paths, and sorts output", async () => {
      ...
      Reflect.set(
        ...
        vi.fn(
          (
            ...
            const normalized = currentPath.toString().replace(/\\/g, "/");
            if (normalized === "/workspace/src") {
              return [
            ...
            if (normalized === "/workspace/src/nested") {
              return [
      ...
      expect(fs.readdirSync).toHaveBeenCalledWith(path.normalize("/workspace/src"), {
        withFileTypes: true,
      });
      expect(fs.readdirSync).toHaveBeenCalledWith(path.normalize("/workspace/src/nested"), {
        withFileTypes: true,
      });
    });
      ...
      Reflect.set(
        ...
        vi.fn(
          (
            ...
            const normalized = currentPath.toString().replace(/\\/g, "/");
            if (normalized === "/workspace/packages") {
