...
export class ExtensionLifecycle {
  ...
  async activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    this.activated = true;
    logger.info("Initializing OpenCode...");
    ...
    try {
      ...
      logger.info("OpenCode activated successfully");
    } catch (error) {
      logger.error(
        `Failed to activate OpenCode: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  async deactivate(): Promise<void> {
    this.outputChannelService?.info("Deactivating OpenCode...");
    ...
    logger?.info("OpenCode deactivated");
  }
