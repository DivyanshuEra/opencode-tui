...
describe("dashboard manager", () => {
  ...
  it("ships OpenCode TUI Manager document and header titles", () => {
    const html = readFileSync(join(__dirname, "dashboard.html"), "utf-8");

    expect(html).toContain("<title>OpenCode TUI Manager v{{HTML_VERSION}}</title>");
    expect(html).toContain('<div class="title">OpenCode TUI Manager</div>');
  });
